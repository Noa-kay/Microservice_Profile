# MyProject – .NET 7+ Web API

A clean, decoupled N-Layer Web API built with .NET 7+.

---

## Project Structure

```
MyProject/
├── Controllers/        # HTTP endpoints – receives requests, returns responses
├── BLL/                # Business Logic Layer – services and core logic
├── Data/
│   ├── Models/         # Database entities (EF Core)
│   ├── Context/        # DbContext definition
│   └── Repositories/  # Data access queries (optional)
├── DTOs/               # Data Transfer Objects – API contracts
└── Migrations/         # Auto-generated by EF Core
```

---

## Getting Started

```bash
dotnet new webapi -n MyProject
cd MyProject
mkdir Data Data/Models Data/Context Data/Repositories BLL DTOs
```

---

## Data Flow

```
Database → Entity → Repository → Service (BLL) → DTO → Controller → Client
```

---

## Team Roles

| Role | Layer | Responsibility |
|---|---|---|
| Data Developer | `Data/` | DbContext, Entities, Migrations |
| BLL Developer | `BLL/` | Interfaces & Service implementations |
| DTOs & Mapping Developer | `DTOs/` | DTOs + AutoMapper configuration |
| Controllers Developer | `Controllers/` | Endpoints, DI registration, Swagger |

### Recommended Workflow

1. **Data + DTOs developers start first** – define models and contracts
2. **BLL developer** builds services based on the data layer
3. **Controllers developer** wires everything to the API
4. Each developer works on a **separate Git branch** and merges after review

---

## Key NuGet Packages

```bash
dotnet add package Microsoft.EntityFrameworkCore
dotnet add package AutoMapper.Extensions.Microsoft.DependencyInjection
dotnet add package FluentValidation.AspNetCore
```

---

## Service Registration

In `Program.cs`, register your services for Dependency Injection:

```csharp
builder.Services.AddScoped<IUserService, UserService>();
```

---

## Code Examples

### DTO

```csharp
namespace MyProject.DTOs
{
    public class UserDto
    {
        public int Id { get; set; }
        public string Username { get; set; } = string.Empty;
    }
}
```

### Service Interface & Implementation (BLL)

```csharp
namespace MyProject.BLL
{
    public interface IUserService
    {
        IEnumerable<UserDto> GetAllUsers();
    }

    public class UserService : IUserService
    {
        public IEnumerable<UserDto> GetAllUsers()
        {
            // Fetch from Data layer and map to DTOs
            return new List<UserDto>();
        }
    }
}
```

---

## Pro Tips

- **Interfaces** – Always define an interface per service to enable easy unit testing and swapping implementations.
- **AutoMapper** – Automates mapping between Entities and DTOs, eliminating repetitive manual mapping code.
- **FluentValidation** – Keeps validation logic clean and decoupled from DTOs.
- **Global Usings** – Use a `GlobalUsings.cs` file to avoid repetitive `using` statements across files.

---

## 🏗️ Business Logic Layer (BLL) – Noa's Part (חלק 2)

שכבת ה-BLL מהווה את הליבה הלוגית של המערכת. היא אחראית על עיבוד הנתונים, אכיפת חוקים עסקיים, וביצוע הפרדה מוחלטת בין שכבת ה-Data (Entities) לבין שכבת ה-Presentation (Controllers/DTOs).

### 🛠️ עקרונות ועבודה עם Clean Architecture

- **Interfaces First** – עבודה מול ממשקים המאפשרת הזרקת תלות (Dependency Injection) ובדיקות יחידה קלות.  
- **DTO Mapping עם AutoMapper** – מניעת חשיפה של מודלים מה-DB החוצה ושימוש ב־`MappingProfile` לריכוז כל ה־CreateMap.  
- **Service Orchestration** – ריכוז לוגיקה מורכבת (כמו ניהול קשר Many-to-Many של Skills) בתוך שירותים ייעודיים.

### 📂 מבנה תיקיית `BLL`

| תיקייה | תפקיד |
| :--- | :--- |
| `Interfaces` | הגדרת ה-Contracts עבור ה-Services וה-Repositories. |
| `Repositories` | מימוש הגישה לנתונים (כולל Generic Repository). |
| `Services` | מימוש הלוגיקה העסקית, ניהול קבצים ואימות נתונים. |
| `Mapping` | הגדרת פרופילי המיפוי של AutoMapper. |

### ⚙️ פיצ'רים מרכזיים

1. **ניהול פרופיל וכישורים (Skills)**  
   - ניהול ישות `PersonalDetails` המקושרת ל־`User`.  
   - לוגיקת עדכון כישורים המטפלת אוטומטית ב־`SkillToUser` – הוספה והסרה בשיחת API אחת.

2. **מערכת ניהול פרויקטים**  
   - CRUD מלא לפרויקטים של סטודנטים.  
   - אינטגרציה עם `FileService` לשמירת תמונות פרויקט תחת `wwwroot/uploads`.

3. **אימות נתונים (Validation)**  
   - אימות Email באמצעות Regex.  
   - בדיקת תקינות URLs (למשל GitHubLink).  
   - הגבלת העלאת קבצים לסוגי תמונות בלבד ועד נפח של 5MB.

### 🔧 התקנה והרצה (BLL)

ה-BLL משתמש בחבילת NuGet:

```bash
dotnet add package AutoMapper.Extensions.Microsoft.DependencyInjection
```

רישום השירותים לדוגמה ב־`Program.cs`:

```csharp
builder.Services.AddScoped<IStudentService, StudentService>();
builder.Services.AddScoped<IProjectService, ProjectService>();
builder.Services.AddAutoMapper(typeof(MappingProfile));
```

### 🔌 Endpoints רלוונטיים ל-BLL

| Controller | Method | Endpoint | Description |
| :--- | :--- | :--- | :--- |
| **Student** | GET | `/api/student/{userId}` | שליפת פרופיל סטודנט מלא כולל Skills |
| **Student** | PUT | `/api/student` | עדכון פרטים אישיים וניהול רשימת כישורים |
| **Project** | GET | `/api/project/user/{userId}` | שליפת כל הפרויקטים של משתמש ספציפי |
| **Project** | POST | `/api/project/user/{userId}` | הוספת פרויקט חדש (כולל תמונה) |
| **Project** | DELETE | `/api/project/{id}` | מחיקת פרויקט קיים |
| **File** | POST | `/api/file/upload` | העלאת קבצים עם ולידציית סוג וגודל |

### 👩‍💻 אחריות

שכבת ה־**BLL (חלק 2)** פותחה על ידי **נועה**, בהתאם לחלוקת התפקידים המוגדרת בטבלת ה־Team Roles למעלה.

---

## API Security (Without Login Flow)

This project validates JWT access tokens and protects endpoints by default.

### Security behavior

- All API controllers require an authenticated user (`FallbackPolicy` + `[Authorize]`).
- JWT tokens are validated for issuer, audience, signing key, and expiration.
- Middleware order is configured as `UseAuthentication()` before `UseAuthorization()`.
- Swagger supports `Bearer` auth so protected endpoints can be tested directly.

### 401 / 403 responses

- **401 Unauthorized** is returned when the request has no token, invalid token, or expired token.
- **403 Forbidden** is returned when a token is valid but the user does not have required permissions/role.
- Both responses are returned as JSON payloads for consistent API handling.

### JWT configuration

Set the following values in `appsettings.json`:

```json
"Jwt": {
  "Issuer": "student-profile-api",
  "Audience": "student-profile-client",
  "Key": "REPLACE_WITH_A_LONG_RANDOM_SECRET_KEY_MIN_32_CHARS"
}
```

### Example test flow

1. Call any protected endpoint **without** `Authorization` header -> expect `401`.
2. Call with invalid/expired `Bearer` token -> expect `401`.
3. Call `POST /api/users` with valid non-admin token -> expect `403`.
4. Call with valid token and required role claims -> request succeeds.
